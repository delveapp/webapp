class UpdateScoreProcedure < ActiveRecord::Migration
  def self.up
    ...
    sql = <<-SQL
      ...
      WHERE menu_item_scores.id=(SELECT menu_item_score_id FROM menu_items WHERE NEW.menu_item_id=menu_items.id);
    SQL
